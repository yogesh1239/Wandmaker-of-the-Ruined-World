...
3. For each candidate name/term/character, `Grep` `glossary.md`, `character-reference.md`, and `character-voices.md` for it before writing — a targeted grep tells you whether it's a duplicate as well as a full read would. Do **not** read these three files in full; they grow every batch and most of any one file is irrelevant to any one candidate. The one exception: the Running Summary window trim (below) genuinely needs the whole `style-guide.md` Running Summary section to see what falls outside the window — read that section only, not the rest of the file.
4. `Grep` `novel.config.md` for the series-identity fields and any recorded official EN release or wiki — not a full read.
5. Web-verify each genuinely new entry (below), then update the three reference files, add only what actually appears in this chapter's text.
