...
Populate the remaining knobs per `core/schemas/novel-config-schema.md`: source→volume map, `Source/`/`English/`/`Editing/` paths, romanization (no-macron, vowel-doubling), reading direction (source RTL → build LTR), part-split threshold, and EPUB metadata.
