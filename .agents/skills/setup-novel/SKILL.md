...
One-time bootstrap that turns the empty harness into a live project for this novel. Run once per novel; re-run to extend to a later volume. You drive the sequence and dispatch the per-stage agents in `.codex/agents/` **by name** (`updater`, `translator`) — spawn → wait → collect → close. Subagents are ephemeral and isolated; there is no shared task board, so ordering lives in this prose. Dispatch at most **6 subagents concurrently**. Verify every output **on disk** before you treat a step as done — an agent returning is not proof of a finished file.
...
Populate the remaining knobs per `core/schemas/novel-config-schema.md`: source→volume map, `Source/`/`English/`/`Editing/` paths, an explicit no-macron romanization lock (including whether long vowels are doubled or unmarked), reading direction (source RTL → build LTR), part-split threshold, and EPUB metadata.
